    ...
    var backgImage,focusImage,clickImage,allImage;
    var postType;
    var num;
    ...
    function modalReset(){
        ...
        if( backgImage == undefined || backgImage == "" || backgImage == "null"){
            $("#staticBackgroundPicForm .imgUpload").remove();
        }else{
            if($("#staticBackgroundPicForm .imgUpload").length > 0){
                $("#staticBackgroundPicForm .imgUpload a").attr("href",backgImage);
                $("#staticBackgroundPicForm .imgUpload a").text(backgImage);
            }else{
                $("#staticBackgroundPicForm").append('<p class="imgUpload col-xs-8">已上传：<a href="'+backgImage+'" target="_blank">'+backgImage+'</a></p>')
            }
        };
        $("#staticBackgroundPicForm .tablesorter").bind('DOMNodeInserted', function() {
            $("#staticBackgroundPicForm .imgUpload").remove();
        });
        // 聚焦
        focusImage = $("#columnId").parent().attr("focusBGImg");
        if( focusImage == undefined || focusImage == "" || focusImage == "null"){
            $("#focusBackgroundPicForm .imgUpload").remove();
        }else{
            if($("#focusBackgroundPicForm .imgUpload").length > 0){
                $("#focusBackgroundPicForm .imgUpload a").attr("href",focusImage);
                $("#focusBackgroundPicForm .imgUpload a").text(focusImage);
            }else{
                $("#focusBackgroundPicForm").append('<p class="imgUpload col-xs-8">已上传：<a href="'+focusImage+'" target="_blank">'+focusImage+'</a></p>')
            }
        };
        $("#focusBackgroundPicForm .tablesorter").bind('DOMNodeInserted', function() {
            $("#focusBackgroundPicForm .imgUpload").remove();
        });
        // 点击
        clickImage = $("#columnId").parent().attr("clickBGImg");
        if( clickImage == undefined || clickImage == "" || clickImage == "null" ){
            $("#clickBackgroundPicForm .imgUpload").remove();
        }else{
            if($("#clickBackgroundPicForm .imgUpload").length > 0){
                $("#clickBackgroundPicForm .imgUpload a").attr("href",clickImage);
                $("#clickBackgroundPicForm .imgUpload a").text(clickImage);
            }else{
                $("#clickBackgroundPicForm").append('<p class="imgUpload col-xs-8">已上传：<a href="'+clickImage+'" target="_blank">'+clickImage+'</a></p>')
            }
        };
        $("#clickBackgroundPicForm .tablesorter").bind('DOMNodeInserted', function() {
            $("#clickBackgroundPicForm .imgUpload").remove();
        });
        // 事件类型
    ...
    $(function(){
        ...
        $.post(base+"/platform/scene/basic/sceneLanguage", {id:$("#sceneBasicInfoId").val()}, function (data) {
            ...
            for(var g = 0; g < language.length; g++){
                $("#languageName").append('<label class="col-xs-3 control-label">'+language[g].languageDic.name+'名称：</label>' +
                    ...
                    '</div>');
                // alert(language[i].sceneLanguageDic.name+"名称："); //input  text  //input hidden language[i].languageDicId
                // console.log(language[i].sceneLanguageDic.name+"名称：" +language[i].languageDicId);  .split("Event")[0]
                // console.log($("input#"+g).val());
        ...
        $("#overallEdit").on("click",function(){
            ...
            $("#overallBackgroundPicForm .tablesorter").bind('DOMNodeInserted', function() {
                $("#overallBackgroundPicForm .imgUpload").remove();
            });
        });
        ...
        $("#overall").on("click",function(){
            ...
            $(".drag").css({"background-image":"url("+dragBGUpload+")","background-color":"#"+$("#overallBG").val() || "","font-size":$("#overallFont").val(),"color":"#"+$("#overallColor").val() || ""});
        ...
        // 添加推荐位
        $("#recommendDrop").on("click",function(){
            var sel = $(".hdDown li").find("p.on");
            if(sel.length < 1){
                Toast.error("请添加栏目！");
            }else if(sel.parent().attr("rule") == undefined){
                Toast.error("请先操作栏目！");
            }else if(sel.parent().attr("rule") != "null" && sel.parent().attr("rule") != undefined){
                Toast.error("当前不可操作！");
            }else{
...
//                $(".hdDown li").eq(0).trigger("click");
        ...
        $("#columnDetail").on("click","#columnEdit",function(){
            ...
            //  本容器，自定义的列表，电视信号，已有一条数据
            if($(".selectNum").find("option:selected").val() != "Container"
                && $("input[name='custom']:checked").val()!= "customList"
                && $(".selectNum").find("option:selected").val()!= "GeneralTVSignal" && $(".selectNum").find("option:selected").val()!= "SpecialTVSignal"
                && (!$("tr").hasClass('selected') && $(".selectNum").find(".optionSel").length < 1 )){
                alert("请选择一条事件类型数据");
                return;
            };
            ruleId();
