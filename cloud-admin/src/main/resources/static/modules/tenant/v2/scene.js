    ...
    function selectNum(){
        $(".selectNum").on("click","select",function(){
            ...
            $(".selectTab").show();
            $(".selectTab .app,.selectTab .custom,.selectTab .generalTVSignal,.selectTab .specialTVSignal").hide();
    ...
    // ruleID   e为select的value值，p为自定义input判断
    function ruleId(e,p){
        switch (e) {
            case "Recommended":// 容器
                ruleText = "推荐位容器";
    ...
    function publicGetName(nameClass,bool){
        if(postType=="Recommended"||postType=="Menu"||postType=="Show"||postType=="Menu"||postType=="SpecialTVSignal"||postType=="GeneralTVSignal"||(postType=="Custom"&&cusInput=="customList")){
            if(bool != true){
        ...
        var getNameRule;
        if(postType=="Custom"&&(cusInput=="customMedia" || cusInput=="customMediaTopics")){
    ...
    function columnDouble(){
        ...
        // $(".dragBox").removeAttr("ondblclick");
        $("#accordionPanel,#accordionMenu,#newMenu,.pendantInfoHide").hide();
        $("#accordion,#accordionStyle,#accordionEvent,.pendantInfo").show();
        $(".selectNum").find("option[date-mark='mark']").remove();
        $(".selectNum select").prepend("<option date-mark='mark' value='Show'>内容展示容器</option>");
        $(".selectNum select").prepend("<option date-mark='mark' value='Menu'>导航菜单容器</option>");
        $(".selectNum select").prepend("<option date-mark='mark' value='Recommended'>推荐位容器</option>");
        $(".indexSort,.iconEdit").show();
        $("#publicDetail").modal("show");
        modalReset();
    };
    // 推荐双击
    function childDouble(){
        ...
        $("#accordionPanel,#accordionMenu,#newMenu,.pendantInfoHide").hide();
        $("#accordion,#accordionStyle,#accordionEvent,.pendantInfo,.indexSort").show();
        $(".selectNum").find("option[date-mark='mark']").remove();
        $(".iconEdit").hide();
        $("#publicDetail").modal("show");
        modalReset();
    ...
    function dragBoxDouble(){
        ...
        $(".indexSort,.iconEdit,.focusChange,.pms").hide();
        $(".ShowUI,.ImageUI").show();
    ...
    function speciesBoxDouble(){
        ...
        if($(".speciesBox.addSpeciesTo").attr("type") == "MenuUI"){ // 菜单
            ...
            $("#accordionPanel,#accordionMenu,#accordionStyle,#accordionEvent,#newMenu,.ShowUI").show();
            $(".selectNum").find("option[date-mark='mark']").remove();
        ...
        $(".languageMenu input,.indexSort,.iconEdit,.pms").hide();
    ...
    $(function(){
        ...
        $.post(base+"/platform/tenant/scene/tenantLanguage", {id:$("#tenantId").val()}, function (data) {
            ...
            for(var g = 0; g < language.length; g++){
                $(".languageName").append('<label class="col-xs-3 control-label">'+language[g].languageName+'名称：</label>' +
                    ...
                    '</div>');
                $(".newMenuLanguage .selectMenu").append("<option value='"+g+"' id='"+language[g].languageDicCode+"'>"+language[g].languageName+"</option>");
                $(".newMenuLanguage .languageMenu").append("<input type='text' class='form-control' placeholder='"+language[g].languageName+"' />");
        ...
        $(".setClick").on("click",function(){
            ...
            var dataTo = $(".selectNum").find("option:selected").attr("data-to");
            if($("."+dataTo).find("tr").hasClass("selected") || $(".dataTableMedia,.dataTableMediaSubject").find("tr").hasClass("selected")
                || $(".selectNum").find("option:selected").val() == "Recommended"
                || $(".selectNum").find("option:selected").val() == "Menu"
                || $(".selectNum").find("option:selected").val() == "Show"
                || $(".selectNum").find("option:selected").val() == "GeneralTVSignal"
                || $(".selectNum").find("option:selected").val() == "SpecialTVSignal"
                || ($(".selectNum").find("option:selected").val() == "Custom" && $("input[name='custom']:checked").val() != "customMedia" && $("input[name='custom']:checked").val() != "customMediaTopics")){
                ruleId(selVal);
    ...
    function saveSubmit(){
        ...
        for(var i = 0; i < columnList.length; i++){
            ...
            for(var e = 0; e < recomList.length; e++){
                if(recomList.eq(e).hasClass("dragBox")){
                    ...
                    // 推荐位内挂件面板
                    var recomBox = {
                        "id": recomBoxList.parent().attr("id") || uuid(),
                        "text": "", // 挂件
                        "isAuthorize": "0", //容器是否向下授权1是 0否
                        "style": {
                            "width": recomBoxList.parent().width(),          //宽
                            "height": recomBoxList.parent().height(),         //高
                            "parentLeft": recomBoxList.parent().position().left,     //相对父窗口位置
                            "parentTop": recomBoxList.parent().position().top,       //相对父窗口位置
                            "fontColor": recomBoxList.parent().css("color"),  //文字颜色
                            "fontSize": recomBoxList.parent().css("font-size"),   //字体大小
                            "backGroupColor":  recomBoxList.parent().css("background-color"), //背景颜色
                            "backGroupImage": recomBoxList.parent().attr("backGroupImage")    //背景图片
                        },
        ...
        for(var b = 0; b < dragBoxList.length; b++){
            if(dragBoxList.eq(b).children(".pendantChild").length < 1 && dragBoxList.eq(b).children(".columnBox").length < 1){
                var dragBoxDiv = {
    ...
    function previewSave(){
        ...
        $(".dragBox").each(function(){
            ...
            if(dragBox.attr('data-id') == 'boxId'){
                ...
                dragBox.attr({"backGroupImage": previewBGUpload,"nameList": JSON.stringify(nameMap) || null,"roll": $(".roll").find("input[name='roll']:checked").val()});
