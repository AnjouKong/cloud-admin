...
@Controller
@Api(tags ="媒资聚合")
@RequestMapping("/platform/media/converge")
public class MediaConvergeController extends BaseController {
    @Autowired
    ...
    @RequestMapping("")
    @ApiOperation("首页跳转")
    public String index(HttpServletRequest req, String id,String levelId,
...
//        req.setAttribute("funcs",deviceBrandVersionFuncService.findFuncs("1"));
        ...
        return "app/media/converge/index";
    }
...
//        req.setAttribute("funcs",deviceBrandVersionFuncService.findFuncs("1"));
        req.setAttribute("levelId",levelId);
        return "app/media/converge/selectMediaSeries";
    }
    ...
    @RequiresPermissions("sys.manager.unit")
    @ApiOperation("编辑数据")
    public Result editDo(HttpServletRequest req,MediaConverge mediaConverge) {
        if(mediaConverge!= null){
            List<MediaConverge> mediaConvergeList = new ArrayList<>();
            String[] seriesId =  mediaConverge.getSeriesId().split(",");
            String[] seriesName =  mediaConverge.getSeriesNames().split(",");
            List<MediaConverge> ss = mediaConvergeService.findByRuleCodeAndMergeId(mediaConverge.getRuleCode(),seriesId[0]);

            List<String> funcList = new LinkedList<String>();

            for(int i=0;i<seriesId.length;i++){
                MediaConverge temp = new MediaConverge();
