...
public class ShiroUtil {
    ...
    public static List<SysMenu> getRootMenuList() {
        return getMenuListByParentId(SysMenu.ROOT_PARENT_ID);
    }

    public static List<SysMenu> getMenuListByParentId(String parentId) {
        if (StringUtils.isBlank(parentId)) {
            parentId = SysMenu.ROOT_PARENT_ID;
        }
//        SystemAuthorizingRealm.Principal user = cn.phenix.cloud.security.SecurityUtils.getPrincipal();
        Principal user = cn.phenix.cloud.security.SecurityUtils.getPrincipal();
        List<SysMenu> menuList;
        SysMenuService sysMenuService = SpringContextHolder.getBean(SysMenuService.class);
        assert user != null;
        if (user.isAdmin()) {
            menuList = sysMenuService.findMenuList(parentId);
        } else {
            SysUser sysUser = new SysUser(user.getId());
            menuList = sysMenuService.findMenusByUser(sysUser, parentId, true);
        }
        return menuList;
    }

    public static SysUser getUser() {
//        SystemAuthorizingRealm.Principal principal = cn.phenix.cloud.security.SecurityUtils.getPrincipal();
